...
const Card: React.FC<ICardProps> = ({ cardInfo }) => {
  ...
  return (
    <div className="lg:min-h-[35rem] w-full group/main flex flex-col justify-between rounded-xl border-[1px] lg: m-4 hover:shadow-2xl transition-transform duration-600 hover:-translate-y-2">
      <div className="relative overflow-hidden rounded-t-2xl flex lg:pt-[100%] pt-[50%]">
      ...
      <div className="flex flex-col py-4 px-6 lg:h-[50%] h-[35%]">
        <span className="lg:text-2xl text-lg hover:text-[#1CA8CB] lg:py-2">
          {cardInfo.title}
        </span>

        <div className="lg:text-xl text-sm py-2 flex items-center">
          <div className="flex">
            {ratingStar?.map((i) => {
              return (
                <svg
        ...
        <div className="flex py-2">
          <span className="lg:text-xl text-lg font-bold">${cardInfo.price}</span> /
          <span className="lg:text-xl text-lg text-gray-400">{cardInfo.type}</span>
        </div>
        <div className="flex justify-between py-3">
          <div className="flex items-center">

            <MdOutlineWatchLater size={22} color="#113D48" />
            <span className="lg:text-xl text-lg text-[#113D48]">{cardInfo.duration}</span>
          </div>
